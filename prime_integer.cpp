...
while(true)
	{
				...
				long converted = strtol(m, &p, 10);//конвертация строки в long int
					if (*p) //если строка не конвертится в int, то выполняется код ниже
						...
						y=(int)converted;
						Sieve_Eratosthen * MyInteger = new Sieve_Eratosthen(y); //у меня почему-то не работает без явного приведения через переменную,т.е. если писать Sieve_Eratosthen * p = new Sieve_Eratosthen(int)converted)
